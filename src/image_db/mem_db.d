...
class MemDb : BaseDb
{
	...
	bool addImage(ImageSigDcRes img, bool throwOnAlreadyHave = true)
	{
		user_id_t user_id = m_id_gen.next();
		return addImage(img, user_id, throwOnAlreadyHave);
	}

	bool addImage(ImageSigDcRes img, user_id_t user_id, bool throwOnAlreadyHave = true)
	{
		auto idimg = ImageIdSigDcRes(user_id, img.sig, img.dc, img.res);
		return addImage(idimg, throwOnAlreadyHave);
	}

	bool addImage(ImageIdSigDcRes img, bool throwOnAlreadyHave = true)
	{
		immutable user_id = img.user_id;
		if(user_id in id_intern_map)
		{
			if(throwOnAlreadyHave) {
...
unittest {
	...
	catch(MemDb.AlreadyHaveIDException e)
	{
		thrown = true;
	}
